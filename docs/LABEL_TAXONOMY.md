...
**版本**: v2.0
**更新时间**: 2026-01-06
**总计**: 200 种细分标签（含 3 种干扰类型标签）

> **v2.0 更新说明**: 原 `unclear` 标签已正式拆分为 `lowlight`/`blur`/`glare` 三种细分干扰类型。Batch1 和 Batch2 数据集已完成重新标注。
...
## 5️⃣ 干扰类型 (interference) - 3 种

> 用于标记图像质量问题导致无法准确识别的情况（参考 GLARE 论文）

| 标签 | 显示名称 | 图标 | 说明 | 示例场景 |
|------|----------|------|------|----------|
| `lowlight` | 低光照 Lowlight | 🌙 | 光照不足导致图像过暗 | 夜间、隧道、阴影 |
| `blur` | 模糊 Blur | 💨 | 图像模糊导致细节丢失 | 运动模糊、对焦模糊、雨水模糊 |
| `glare` | 眩光 Glare | ☀️ | 强光导致图像过曝 | 阳光直射、逆光、反光 |

### 使用说明

- 当图像存在上述干扰且**无法准确识别标志内容**时，使用对应的干扰类型标签
- 如果干扰程度轻微、仍可识别标志，则正常标注标志类别
- 旧标签 `unclear` 已废弃，自 v2.0 起不再使用

### v2.0 数据集统计

| 数据集 | 总图片数 | 有效标志 | lowlight | blur | glare |
|--------|----------|----------|----------|------|-------|
| Batch1_new | 2000 | 1690 | 9 | 254 | 47 |
| Batch2_new | 3000 | 2842 | 1 | 153 | 4 |
